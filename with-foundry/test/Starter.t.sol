...
contract StarterTest is Test {
  ...
  ///
  /// @param _testName a random string to identify the test by, this is used to create a unique
  /// folder name in the /tmp directory
  ...
  function generateDynamicProof(
    ...
    require(
      _fields.length == _fieldValues.length, "generateProof: Input arrays not the same length"
    );
    ...
    string memory _newProof =
      vm.readLine(string.concat("/tmp/", _testName, "/proofs/with_foundry.proof"));
